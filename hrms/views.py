...
class Employee_Delete(LoginRequiredMixin, DeleteView):
    ...
    def get(self, request, pk):
        form_app = Employee.objects.get(pk=pk)
        form_app.delete()
...
class Attendance_New(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["today"] = local_date
        pstaff = Attendance.objects.filter(Q(status='PRESENT') & Q(date=local_date))
...
class Attendance_Out(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        from datetime import datetime
        local_date = datetime.now().date()
        user = Attendance.objects.get(Q(staff__id=self.kwargs['pk']) & Q(status='PRESENT') & Q(date=local_date))
        local_time = datetime.now().time()
        user.last_out = local_time
        user.first_in = user.first_in
        user.save()
